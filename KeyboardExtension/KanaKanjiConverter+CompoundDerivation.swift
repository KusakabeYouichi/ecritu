...
extension KanaKanjiConverter {
    ...
    static let numericCounterSuffixCandidatesByReading: [String: [String]] = [
        ...
        "じ": ["次"],
        "かげつ": ["か月", "カ月", "ヶ月", "ヵ月", "箇月"],
        "かこく": ["か国", "カ国", "ヶ国", "ヵ国", "箇国", "ケ国"],
        "かしょ": ["か所", "箇所", "カ所", "ヶ所", "ヵ所"],
        // 件 を先に(ユーザ指定 3224: 数百件 > 数百軒。間(けん)は旧単位で頻度が低く、そもそも助数詞表に無い)
        "けん": ["件", "軒"],
        "しゅうかん": ["週間"],
        "じかん": ["時間"],
    ...
    static let numericCounterAllowedSuffixReadingsByPrefixReading: [String: Set<String>] = [
        ...
        "なん": [
            "こ", "かい", "かげつ", "かこく", "かしょ", "けん", "ごう", "ごうしゃ", "しゅうかん", "じかん", "にち", "だい", "にん", "ねん",
    ...
    static let numericMagnitudeCandidatesByReading: [(reading: String, candidate: String)] = [
        ("せん", "千"), ("ぜん", "千"),
        ("ひゃく", "百"), ("びゃく", "百"), ("ぴゃく", "百"),
        // 促音形(百件=ひゃっけん/三百個=さんびゃっこ/六百軒=ろっぴゃっけん)。無いと すうひゃっけん が
        // 数+百間(辞書語)にしか組めず 数百件 が出なかった(ユーザ報告 3224)
        ("ひゃっ", "百"), ("びゃっ", "百"), ("ぴゃっ", "百"),
        ("まん", "万"),
        ("おく", "億"),
        ("ちょう", "兆"),
    ...
    static let nounKanjiSuffixAffixCandidatesByReading: [(reading: String, candidate: String)] = [
        ...
        ("しゃ", "車"),
        // 神格(太陽神/軍神/海神/戦神 等)。名詞+神(しん)は生産的で、辞書は 守護神/軍神 程度しか持たない。
        // 神(しん)単独は辞書 rank10 で postfix 合成の上位 6 件に入らず、太陽シン/太陽芯/太陽新 しか出なかった(3119)
        ("しん", "神")
    ]
    ...
    static let rendakuPlaceMarkerAllowedDigits: [String: Set<Int>] = [
        "ぜん": [3], "びゃく": [3], "ぴゃく": [6, 8], "びゃっ": [3], "ぴゃっ": [6, 8]
    ]
    // 和語数詞(〜つ)。ひとつ→1つ 等。
    ...
    static let digitContextAdditionalCounterSurfacesByReading: [String: [String]] = [
        // 助詞と同音の 1 かな助数詞(と=斗、て/で=手)。本表に置くと 何と/なんて/で の助詞を 何斗/何手 の合成
        // (派生床で安い)が乗っ取る(でなんとでるか→で何斗出るか。2820)。数字の直後(3と→3斗、5て→5手)だけで作る
        "と": ["斗"],
        "て": ["手"],
        "で": ["手"],
        // かな助数詞 つ(1つ/2つ)。1つ 単独はかな識別が先頭で足りるが、つ+付属語(1つも/1つの/1つしか)は
        // 表に つ が無いため「助数詞読み+かな末尾」の合成が組めず、辞書語 ツモ(wc 2148)が先頭に出ていた
        // (1+つも→1ツモ。ユーザ報告 2892)。本表に置くと 何つ/第1つ の複合生成に波及するのでこちらに置く
        "つ": ["つ"],
        ...
        "がん": ["貫"],
        "きゃく": ["脚"],
        "きょく": ["曲", "局"],  // 1曲(楽曲)を先に、1局(対局)は後ろ(ユーザ指定 3051)
        "きれ": ["切れ", "切", "帛"],
        "きろぐらむ": ["瓩"],
        "きん": ["斤", "听"],
        "ぎょう": ["行"],
