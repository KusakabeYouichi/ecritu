...
extension KanaKanjiConverter {
    ...
    func applySameReadingScriptPreference(
        ...
        if let identityScore = scores[reading] {
            ...
            if kanaSkippedBySeed {
                ...
                // かなの席を辞書の漢字候補(上位 3 つ、現在の点の順を保存)に譲り、かなはその直下へ(定数コメント参照。2902/2903)。
                // 1 語だけと入れ替えると、その語が正規化や抑制で消える(ぶし の 五倍子)とかなが先頭に戻り、降格だけだと
                // かなの下に居た活用派生(まえ→舞え/こえ→請え)が 前/声 の上に残る。systemCandidates はモード未適用なので
                // 複数を持ち上げて、どれが残っても漢字が先頭になるようにする
                let kanji = others.filter { KanaKanjiConverter.containsKanjiCandidate($0) && scores[$0] != nil }
                    .sorted { scores[$0]! > scores[$1]! }
                    .prefix(3)
                if !kanji.isEmpty {
                    for (offset, surface) in kanji.enumerated() {
                        scores[surface] = max(scores[surface]!, identityScore - offset)
                    }
                    scores[reading] = min(identityScore, identityScore - kanji.count)
                }
            }
        }
    ...
    // 除き、LM でかながその漢字に 800 以上劣る 69 読み(2902)+第 2 弾 118 読み(2903、下記)。一般規則(LM 劣位のかなを一律降格)は いいね/したら/しって 等
    ...
    static let kanaHarvestDemotedReadings: Set<String> = [
        ...
        "いろう", "せいじゃ", "しょうよう", "かんじん", "ちょうけい", "じょたい", "ころう", "ちょうば", "ほうとう", "ようらん", "ここう", "わこう",
        "いっとう", "ひとう", "きんたい", "いとく",
        // 第 2 弾(2903): 辞書にかな収穫を持つ全 19,964 読みを走査し、かなが先頭に出た 3,237 読みのうち seed/curated/かな優位を除き、
        // 漢字が BCCWJ の名詞・代名詞・形状詞で頻度 100 以上の 147 読みから、かな表記も一般的な語(まぶた/ちまた/はらわた/
        // むちゃくちゃ/もくもく)と多義の短い読み(しょう/おう/しろ/あたい/はち/かあ/ねえ/たか)等 29 を人手で除いた 118 読み
        "まえ", "だれ", "こえ", "あいて", "すがた", "ぜんたい", "かわ", "かず", "はんたい", "しょとく", "そら", "ものがたり", "よぼう", "しかた",
        "あんない", "ちえ", "かって", "あせ", "なっとく", "じんじゃ", "つごう", "りょうて", "おおがた", "ちず", "むだ", "かくとく", "もよう", "ふまん",
        "みぎて", "はたけ", "よめ", "ひだりて", "にがて", "びよう", "どくとく", "せっとく", "おうよう", "すえ", "たな", "どうとく", "ぎゃくたい", "かわせ",
        "きがる", "こどく", "かいたく", "かずかず", "けた", "すもう", "ぶかい", "さた", "つえ", "ぼうず", "やかた", "ちゃわん", "なっとう", "なえ",
        "めいじん", "がけ", "へいたい", "むよう", "てがる", "あいよう", "ひなた", "いちよう", "おやかた", "けいもう", "くつした", "すずめ", "どて",
        "ゆかた", "おおかた", "ぶよう", "もめん", "しつよう", "みちばた", "こんだて", "いおう", "まっちゃ", "はくい", "おきて", "さしず", "はなみず",
        "はんよう", "かっか", "びょうし", "はけ", "はらいた", "くだり", "かぶり", "とうた", "つきよ", "あいどく", "おくがた", "におう", "けいず",
        "たなばた", "さっそう", "ふたまた", "きまじめ", "あまちゃ", "さくそう", "ざった", "あとかた", "こおりみず", "ぎょうじゃ", "じとく", "ばいどく",
        "しおから", "みがる", "はなうた", "ひがた", "ばたい", "させん", "いちず", "ふそう", "ぼうとく", "めいろう", "ぼんよう", "やまがた"
    ]
