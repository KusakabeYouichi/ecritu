...
extension KanaKanjiConverter {
    ...
    static let multiClauseCompoundParticles: Set<String> = [
        ...
        // とか(並立・例示の副助詞): 辞書ではかなが収穫底値(wc 10828)で LM 未収録、渡河/都下 等の名詞に負けて
        // さんこうにしたとかなのかな が と+仮名+のかな に割れていた(ユーザ報告 2740)
        "とか"
    ]
    ...
    // 接頭辞用法の読み(2876)。読みを見ないと 内(ない)+ワイン にも加点され、
    // 「地理的表示のないワイン」が 地理的表示の内ワイン になっていた(抜き取り検査)。
    // 接頭辞として立つのは訓読み(うちポケット/したフリック)で、音読みの 内(ない)は接尾用法
    static let multiClauseDirectionalPrefixReadings: [String: Set<String>] = [
        "下": ["した"], "上": ["うえ"], "左": ["ひだり"], "右": ["みぎ"], "前": ["まえ"],
        "後": ["うしろ", "あと"], "横": ["よこ"], "縦": ["たて"], "内": ["うち"], "外": ["そと"], "逆": ["ぎゃく"]
    ]
    ...
    // なし(無し/ない の文語終止形。口語の「〜なしか」「〜なしよね」): 述語扱いにしないと直後の か が素通り(3565+500)で、
    // どうがもなしか が 動画も名しか(名→しか の bigram)に負けていた(ユーザ報告 3225)
    static let multiClauseKanaPredicateIdentities: Set<String> = ["ある", "する", "かかる", "やる", "なし"]
    ...
    static let multiClauseSeedOrderInflectionBaseReadings: Set<String> = [
        "おくる",  // おくれない seed(送れない/遅れない/贈れない/後れない)の先頭に連文節でもボーナス(2910)
"つかえる", "おく", "たつ", "おきる", "ためす"]
    // いう の未然形(いわず/いわない/いわなくて/いわれる)はかなより漢字(言わ〜)が正書。いう(3293)が
    // 言う(4804)より LM 優位なためかな活用形も供給され、OOV 同点で列挙順のかなが勝っていた
    // (いわずに→いわずに。2881 ユーザ報告)。同点を割るだけの小さな減点。という/そういう(かなが正書)は
    // 未然形でないので触れない
    static let multiClauseKanaMizenIuPenalty = 300
    ...
    // くる: きて/きてます が 着る族(ichidan ルールが先)と名詞の後で同点(派生定額)になり 種類着てます が先頭だった。
    // LM は 来る 5510 < 着る 6530 で日常頻度も 来る が上(ユーザ報告 2910)。を の後(服を着て)は下の prefix pair bonus で 着 を守る
    ...
    static let multiClauseRareReadingFloorExemptSurfacesByReading: [String: Set<String>] = [
        ...
        "ぎふ": ["岐阜"],
        // 縁(ふち) は unigram 5639 で 淵 6438 より安いのに、読み別 wc 7553(主読み えん の統計)で床上げされ
        // ふちのこうか→淵の効果 になっていた(ユーザ報告 3108)
        "ふち": ["縁"],
        // 段(だん) は unigram 5091 の常用語なのに読み別 wc 8733 で床上げされ、ダン(wc 5385)に負けて
        // だんでならびます→ダンで並びます になっていた(マニュアル検査 14 件、2890)
        "だん": ["段"],
        // 時(じ) は unigram 3807 の最頻出語なのに読み別 wc 6156 で床上げされ、字(wc4066)に
        // 一律負けて 申し込み時 が 申し込み字 になっていた(名詞+時 接尾の一般対策。2535)
        "じ": ["時"],
        // 後(ご) は 受諾後/終了後/確認後 の接尾で常用だが、読み別 wc 6845 が最安読み あと(3995)から 2850 乖離して
        // 床上げされ、語(wc 4401)に常敗していた(じゅだくご→受諾語。ユーザ報告 3054)
        "ご": ["後"],
        ...
        "くい": ["食い"],
        // 水(すい) は名詞に付く接尾(昆布水/炭酸水/レモン水。unigram 4621)なのに読み別 wc 7429 で床上げされ、
        // 錐(wc 7409、主読み)に 20 差で負けて こんぶすい→昆布錐 になっていた(ユーザ報告 2833)
        "すい": ["水"],
        // 例(れい) は unigram 4236 の常用語で Sudachi の読みも れい 1 つだけなのに wc 7566 で床上げされ、
        // 霊(wc 5656、uni 5845)に約 170 差で負けて しんだれいも→死んだ霊も になっていた(ユーザ報告 2837)
        "れい": ["例"],
        // 名(めい) は名詞に広く付く接尾(議員名/会社名/商品名/店名。unigram 4025)なのに読み別 wc 6830 で床上げされ、
        // 姪(wc 4230、主読み)に負けて ぎいんめい→議員姪 になっていた(ユーザ報告 2854)
        "めい": ["名"],
    ...
    // 名詞+かん は接尾 感(サイズ感/スピード感/距離感)。LM は 官 5297 < 観 5446 < 感 5626 < 缶 5866 で
    // 感 が中位、さらに カン→は 1114 が 感→は 1780 より安いため さいずかんは が サイズカンは になっていた
    // (ユーザ報告 2850)。よく使う複合(スピード感 7079/距離感 7302/違和感 6347、逆に 空き缶 7489/図書館)は
    // LM が 1 語として持つので分割経路が勝たず、この加点は「LM の知らない新しい複合」にだけ効く。
    // 前の語→缶/館/管/観/官 が観測済み(映画→観/館 等の実在複合)なら触らない
    static let multiClauseFeelSuffixCompetitorSurfaces: [String] = ["缶", "館", "管", "観", "官", "巻", "韓"]
    static let multiClauseFeelSuffixAfterNounBonus = 1200
    // 名詞+ごと は接尾(括弧ごと/入力モードごと/秒ごと)。表記はかなが本則で、漢字の 毎 は使わない。
    // LM は かな ごと 4766 に対し 語 4085 が安く、括弧ごと入力 が 括弧+語+と+入力 に 1 点差で
    ...
    // 格助詞 が/を の直後に係助詞・格助詞(は/も/が/を)が続く「がは」「をも」は非文(引用の「〜が」は 等を除く)。
    // まっくののほうがはやくから が 方が+は+約+から に割れて 早く が出なかった(ユーザ報告 2740)
    static let multiClauseParticleAfterCaseParticlePenalty = 2500
    // 上の減点の「直前」として認める、格助詞 が/を で終わる短いかな/漢字かな句(ほうが=方が 等の複合ノード込み)。
    // まんが(漫画)+は のような が 終わりの名詞は含めない
    static let multiClauseCaseParticleEndingPhrases: Set<String> = ["が", "を", "ほうが", "方が", "のが", "のを", "ことが", "ことを", "ものが", "ものを"]
    ...
    // 助詞を呑んだ派生動詞(でなおす=出直す / で+直す)の代替経路(2771、適用箇所のコメント参照)。
    ...
    static let multiClauseNumeralCharacters: Set<Character> = Set("0123456789０１２３４５６７８９〇一二三四五六七八九十百千万数何幾")
    // 並列動詞の表記整合(2771、適用箇所のコメント参照): 漢字を揃えた経路が元の混在経路より
    // この差までなら揃えた方を最良にする(かうかかわないか: 揃え 買う…買わない は +100 程度)
    static let multiClauseCoordinatedVerbMaxDelta = 3000
    // 体言直後のかなコピュラ・クラスタ(だし/だから/だけど…。2771): 体言+だ は最も基本的な述定で、
    // 続く接続助詞・終助詞までかなが正書。ぴんくだし が ピン+下し(下し 5852)に負けて
    // ピン下し が先頭だった。述語直後の終助詞クランプ(multiClauseFinalParticleReadings)と同じ
    // 水準(1200)へクランプする。直前がカタカナ語(助詞・述語は除く)のときだけ — 漢字名詞に掛けると
    // 同音の誤変換名詞(層/奴/遭難/何処)がかな正書に勝つため(適用箇所のコメント参照)
    static let multiClauseNounCopulaClusterReadings: Set<String> = [
        "だし", "だから", "だけど", "だけれど", "だって", "だよ", "だね", "だよね", "だな", "だなあ",
        "だろう", "だろ", "でしょ", "でしょう", "ですし", "ですね", "ですよ", "ですよね", "ですが", "ですけど"
    ]
    // 楽器名(2771): 直後の 弾く 系(弾ける/弾いて/弾く…)を同音の 引く/曳く 系より優遇する。
    // 楽器名+を+弾く は LM の を→弾 で足りるが、助詞を省いた口語(チェロひける)は名詞直後の派生が
    // 一律 7200 で辞書順(曳ける)に落ちていた
    static let multiClauseInstrumentNounSurfaces: Set<String> = [
        "ピアノ", "ギター", "バイオリン", "ヴァイオリン", "ビオラ", "ヴィオラ", "チェロ", "コントラバス", "ハープ",
        "三味線", "琴", "箏", "ウクレレ", "マンドリン", "リュート", "ベース", "エレキ", "二胡", "胡弓", "琵琶",
        "大正琴", "シタール", "バンジョー", "オルガン", "キーボード", "チェンバロ", "ハープシコード", "ピアニカ",
        "鍵盤", "エレキギター", "アコギ", "アコースティックギター", "クラシックギター", "ベースギター", "ハーモニカ"
    ]
    ...
    static let multiClauseAcrossNoCollocationBonuses: [String: Int] = [
        ...
        // LM も 振っ 6479 < 降っ 6550。雨/雪 の後だけ 降 に寄せる(表層接頭一致なので活用形全体に効く)
        "雨\t降": 2500,
        "雪\t降": 2500,
        // 旗+振(はたをふって→端を振って。2882、ユーザ報告)。端(5479、端→を 684)が 旗(5796、旗→を 989)に勝つ
        "旗\t振": 2500,
        // 橋+渡(はしをわたる→端を渡る。同型)
        "橋\t渡": 2500,
        // 割合の語+占(はんぶんをしめています→半分を締めています。2884、抜き取り検査)。しめ の辞書は
        // 締め だけで 占め が無く、活用派生の を→占めています/締めています は定額 5000 で同点、
        // 第 2 キーの辞書 rank で 締め が勝つ。占める の目的語になる割合・面積・順位の語の後だけ寄せる
        // 方位の 北+方位・順序の語(きたからみなみへ→来たから南へ、きたとひがし→来たと東。2885、抜き取り検査 32 件)。
        // 来た は misc の curated(床 1500、文頭 700)で 北(4646)に常勝する(文頭 700 vs 北 5146 の差 4400 を覆す幅)。から/と は 来たから(理由)が一般文では
        // 普通なので、方位語が続くときだけ 北 に寄せる(は/を の前と の+文末 は DP 側で 来た を減点)
        "北\t南": 5500,
        "北\t東": 5500,
        "北\t西": 5500,
        "北\t北": 5500,
        "北\t順": 5500,
        // 季節・気候の語+暑(なつはあついきこう→夏は厚い気候。ユーザ指定 2885。隣接側は 暑い\t気候 参照)
        "夏\t暑": 2500,
        "気候\t暑": 2500,
        "生育期\t暑": 2500,
        // コスト・費用の語+抑(こすとをおさえた→コストを押さえた。ユーザ指定 2885)。文脈が無ければ 押さえ が
        // わずかに優先(を→抑え 3973 < を→押さえ 4904 だが派生ノードは定額で unigram 順)。金銭の語の後だけ 抑
        // 列や並びの語+先頭(こうほのせんとう→候補の戦闘。マニュアル検査 2890)。の→戦闘 4074 < の→先頭 4404
        "候補\t先頭": 2500, "行\t先頭": 2500, "文\t先頭": 2500, "リスト\t先頭": 2500, "配列\t先頭": 2500,
        "文字列\t先頭": 2500, "単語\t先頭": 2500, "列\t先頭": 2500, "語\t先頭": 2500,
        // 入力欄の語+欄(なまえのらんに→名前の乱に。同上)。の→乱 3611(応仁の乱)≪ の→欄 4832
        "名前\t欄": 2500, "氏名\t欄": 2500, "備考\t欄": 2500, "入力\t欄": 2500, "住所\t欄": 2500, "検索\t欄": 2500,
        "コメント\t欄": 2500, "メモ\t欄": 2500, "上\t欄": 2500, "下\t欄": 2500, "右\t欄": 2500, "左\t欄": 2500,
        // 数字・文字+入る(すうじがいります→数字がいります。同上)。かな いります(要る)が先頭だった
        "数字\t入": 2000, "文字\t入": 2000, "記号\t入": 2000, "候補\t入": 2000,
        // 身体の語+押さえ(てをおさえた→手を抑えた。ユーザ指定 2889)。を→抑え 3973 < を→押さえ 4904 で
        // 無文脈は 抑え。手/口/耳/目/頭/腹/胸/肩/傷/鼻 の後だけ 押 に寄せる
        // 身体の語+腫れ(じかせんがはれた→耳下腺が晴れた。2893)。が→晴れ 5373 < が→腫れ 6106
        "耳下腺\t腫": 2500, "顔\t腫": 2500, "足\t腫": 2500, "目\t腫": 2500, "喉\t腫": 2500, "手\t腫": 2500,
        "指\t腫": 2500, "瞼\t腫": 2500, "歯茎\t腫": 2500, "扁桃腺\t腫": 2500, "唇\t腫": 2500, "腹\t腫": 2500,
        "手\t押": 2500,
        "口\t押": 2500,
        "耳\t押": 2500,
        "目\t押": 2500,
        ...
        "上位\t占": 2000,
        "位\t占": 2000,
        // しめる の目的語による書き分け(3041、ユーザ報告 しめちゃうらしいぞ)。基底順は 閉める 先頭にしたが、を→占める 3243 の
        // bigram(を→閉める 5708)が無文脈で 占 を押し上げる(ドアを占める)。差 2465 に文末 bigram の差も乗るので 3500。開閉するもの→閉、結ぶもの→締、首→絞
        "ドア\t閉": 3500, "窓\t閉": 3500, "戸\t閉": 3500, "扉\t閉": 3500, "蓋\t閉": 3500, "ふた\t閉": 3500,
        "鍵\t閉": 3500, "カーテン\t閉": 3500, "シャッター\t閉": 3500, "店\t閉": 3500, "引き出し\t閉": 3500,
        ...
        // 仕える の典型的な目的語(神に仕える/主人に仕える)。に→使える の加点(3109)で 使える に倒れないよう 仕 へ寄せる
        "神\t仕": 3000, "主人\t仕": 3000, "主\t仕": 3000, "王\t仕": 3000, "殿\t仕": 3000, "君\t仕": 3000,
        "仏\t仕": 3000, "幕府\t仕": 3000, "将軍\t仕": 3000, "領主\t仕": 3000, "藩\t仕": 3000, "家\t仕": 2500,
        ...
        "凍り\t解け": 800,
        // 温度を計る/測る(ユーザ指定 2801): を→図る 3529 の Wikipedia バイアス(〜を図る)で 温度を図る が先頭。
        ...
        "紙\t印刷": 1500,
        // 保険が効く/保険は効く(ユーザ報告 2850): LM は 聞く 5878 ≪ 効く 6911・利く 6971 で、
        // 保険が聞く/保険が利く が上に来ていた。効く の主語は薬効・制度側で「聞く」ではありえない。
        // 後段は前方一致で引くので 効いた/効かない/効きます も同じ 1 行で拾える
        "保険\t効": 2000,
        // 薬が効く も同じ連語(利く 6971 が 効く 6911 を bigram 差で逆転していた)。現代表記は 効く
        "薬\t効": 2000,
        // 頻繁に起きる: おきます の活用族は 置く 側が族選好(上の
        // multiClauseInflectionFamilyPreferenceBaseReadings)で 800 先行するため、
        // 置きます(12042)が 起きます(12842)に勝つ。単語単位では 起き5006<置き5634、
        // に→起き4160<に→置き4526 と 起き が有利なのに逆転していた。頻繁に は副詞なので
        // 置く の目的語にならない(棚に置きます 型は無傷。ユーザ報告 2973)。
        // surface モードは未解決(起きます が 1 語で供給されず 起き+ます に割れ、
        // ます 4633 の分だけ 置きます(1 語 12042)に届かない)
        "頻繁\t起き": 2400,
    ]
    ...
    static let multiClauseSeedOrderNounBonusesByReading: [String: Int] = [
        ...
        // つりばし: 送り仮名の本則は 吊り橋(BCCWJ 128 回、吊橋 は 0 回)。word_cost は同値 7863 で、
        // LM の 152 点差(吊橋7120 < 吊り橋7272。Wikipedia の橋名 ◯◯吊橋 由来)だけで連文節が
        // 吊橋 を採っていた。seed 先頭を連文節にも通す(2949)
        "つりばし": 400,
        // うる: seed 先頭 売る を連文節でも通す。LM は 得る4907 ≪ 売る6203 で、うるとしたら→
        // 得るとしたら / とちをうる→土地を得る になっていた。文語の 得る(うる)も現代語だが
        // 単独の うる は 売る が主(ユーザ指定 2925)。脱活用スパン用の opt-in 表(800)は
        // 辞書形スパンには効かず、差 1296 にも届かないのでこちらで与える
        "うる": 2000,
        // くると: クルト(Kurt)は Wikipedia 観測で unigram が安く、活用供給の 来ると(派生OOV 7200)に
        // 一律勝っていた。seed 先頭 来ると を連文節でも通す(ユーザ報告 2753)
        "くると": 3000,
        // たて: seed 先頭の名詞 縦 を連文節でも通す。単文節は seed で 縦 が先頭になったが、
        // たてにしたら/たてには は連文節が採られ 立てに が先頭のままだった(ユーザ報告 2755)
        "たて": 2500,
        // おいた: 老いた→とき/ばかり が Wikipedia で観測済みなのに対し 置いた→とき は未観測で、
        // 日常頻度と逆に 老いたときに/老いたばかり が先頭だった(ユーザ報告 2755)
        "おいた": 3000,
        ...
        "にた": 2000,
        // くう: 空(主読み そら)の読み跨ぎ床 7172 が 食う(6911+500)を 239 差で跨ぎ、めもりーくうやつ→
        // メモリー空やつ になっていた(めもりーくう 単体は 空→EOS 未観測で 食う が勝つ)。差を跨ぐ値(2777)
        "くう": 600,
        // きどう: 起動 を連文節でも先頭に。軌道 4585 との unigram 差 870 を跨ぐ(seed コメント参照。2777)
        "きどう": 1200,
        ...
        "こうしゅう": 1500,
        // せんしゅう: 先週(unigram 6896)が 千秋 6200 / 選集 6225 / 専修 6497 に負け、私も先週 が
        // 候補に出なかった(ユーザー報告 3141)。ノード単体の差(696)に加えて、時間副詞ゆえに
        // 先週→EOS が未観測で文末にすると更に 605 不利になるぶんも跨ぐ
        "せんしゅう": 1500,
        // おしい: 惜しい(7489)が 押井(6536)に負けていた。差 953 を跨ぐ値(3182)
        "おしい": 1200,
        ...
        // (2881、ユーザ指定「ちょっとだけ優先」)。彼の味方(彼+の+見方 9977)は 彼+飲み方 より安いまま
        "のみかた": 2200,
        // はっこう: 醗酵 7190 vs 発行 4301。出側 bigram(発行→を 等)も 発行 側にしか無いので広く取る(seed コメント参照。2880)
        "はっこう": 5500,
        // こうし: seed は既に 仔牛 先頭だが連文節は LM で 孔子(5918)/講師(5769)が 仔牛(7649)を
        // 押し出し 孔子の骨付きスネ肉 になっていた。仔牛 を優先(ユーザ指定 2880。講師 は 2 番手に下がる)
        "こうし": 2200,
        // こうしゅうし: 甲州市 7489 vs 広州市 7302(山梨県甲州市→山梨県広州市。2884、抜き取り検査)
        "こうしゅうし": 600,
        // きたはんきゅう: 北半球 は収穫コスト 13513(床 8700 級)で、来た(curated 文頭 700)+阪急 に負ける(2885)
        "きたはんきゅう": 1500,
        // えいじ: 英字 6422 vs エイジ 6073(えいじのにゅうりょく→エイジの入力。マニュアル検査 12 件、2890)
        "えいじ": 800,
        // へんかん: 変換 4799 < 返還 5667 だが から→返還 4598(沖縄返還)で 読みから返還 になる(同 3 件)
        "へんかん": 1000,
        // きゅうせい: 旧姓 6352 vs 急性 5830(ユーザ指定 2910。きゅうせいだった→急性だった)
        "きゅうせい": 1000,
        // かせん: 下線 6940 は 河川 5321/架線 6054/歌仙 6571/寡占 6722/貸せん(派生 7200)に囲まれる(下線付き→河川付き。
        // ユーザ指定 2890)。時+下線 に割れていた 耳下腺 は misc の curated にしたので持ち上げてよい(2891)
        "かせん": 2500,
        // じょうりゅう: 蒸溜 は LM 未収録(蒸留 6510 のみ)。ユーザー指定で 蒸溜 を優先(2879)
        "じょうりゅう": 3000,
        // にっしょく/げっしょく: 日蝕 8139 vs 日食 5855、月蝕 7995 vs 月食 6396(ユーザ方針で 蝕 優先。2896)。
        // 金環日蝕/部分日蝕(astronomique)が 金環+日食 の合成に負けないための幅も兼ねる
        "にっしょく": 3000,
        "げっしょく": 2500,
        // こうみ/かじゅう/ほしょう: 辞書にはあるが LM で日常語・地名に負ける(ユーザー指定 2879)。
        // 小海(地名、こうみ)7059 < 香味 7537 / 荷重 6396 < 果汁 6433 / 保障 5398 < 保証 5502
        // こうみ は 1500 では な→子(4226)+海(2871、海→を 1418)の単漢字連鎖にも負けた
        // (芳醇な香味を形成する→芳醇な子海を形成する。2884、抜き取り検査)
        "こうみ": 2500,
        "かじゅう": 2000,
        "ほしょう": 600,
        // ぶどう: かな正書(ユーザー指定)だが、ブドウ→を 1141 のような出側 bigram の実績が
        // かな側に無く、表外訓の減点(1500)だけでは ブドウを使う に負ける。差(約600)を跨ぐ値(2878)
        "ぶどう": 1200,
        // さんち: 産地 5555 が 山地 5495 に 60 差で負ける(seed コメント参照。2878)
        "さんち": 600,
        // きこう: 気候 5091 が 機構 4758 に負ける。入り側 bigram(が→機構)も安いので
        // 単純な unigram 差(333)より広く取る(seed コメント参照。2878)
        "きこう": 1500,
        // しよう: 使用 は node は安いが出側 bigram で 仕様 に負ける(seed コメント参照。2878)
        "しよう": 600,
        // しゅうと: 州都 が 州+と の分割に負ける(seed コメント参照。2878)
        "しゅうと": 2000,
        // からくち/あまくち: 辛口/甘口 が から+口 / 甘く+力 の分割に負ける。出側 bigram が
        // 無い(未観測=バックオフ)ぶんまで跨ぐ必要があり、他の読みより大きい(seed コメント参照。2878)
        "からくち": 5000,
        "あまくち": 5000,
        // こおり: 郡(ぐん 4705 の借用、→が 1734)が 氷(5429、→が 1142)に 132 差で勝つ。日常語 氷 を優先(2737)
        "こおり": 800,
    ...
    static let multiClauseConstellationStandardSurfacesByReading: [String: String] = [
        "あんどろめだざ": "アンドロメダ座",
        "いっかくじゅうざ": "いっかくじゅう座",
        "いてざ": "いて座",
        "いるかざ": "いるか座",
    ...
    // 述語(活用派生/辞書形)直後の よう は様態・目的の助動詞でかなが正書。名詞接尾の 用 は床上げ免除(2813)で安く、
    // ついかしておくよう→追加しておく用 と立っていた(ユーザ報告 2910)
    ...
    // で(接続助詞の言いさし: はずなのでー/それでー)も伸ばし(はずなのでー→はずなのデー。ユーザ報告 2910)。に/と は なのにー が ナノにー に退行
    static let multiClauseTrailingProlongationPredicateTails: Set<Character> = ["た", "す", "だ", "い", "う", "る", "で"]
    ...
    // 変種枠を文節位置で散らすときのコスト差の上限(2939、ユーザ報告 こしょうのぜんちょうかも)。
    // コスト差順に詰めるだけだと 1 文節が枠を独占し、{故障の全長, 胡椒の全長(100), 呼称の全長(100),
    // 湖沼の全長(256)} と 全長 ばかりで 故障の前兆(451)が 1 つも出ない。1 巡目で「まだ使っていない
    // 文節」の変種を 1 つ採るが、無条件に散らすと表記変種の枠(数か国対応 の ヶ/箇/カ。後段の設定
    // フィルタで 1 つに畳まれる)を奪って 数カ国大王(2963)級のゴミが入る。実測の分離は明快:
    // 採りたい 故障の前兆=451 / 退けたい 数カ国大王=2963・今だと未だ=2808。その間に上限を置く
    static let multiClauseVariantSpreadMaxDelta = 1200
    // サ変名詞+の+事象名詞(故障の前兆/崩壊の前兆/発症の前兆)の加点(2944、ユーザ提案)。
    // 前兆 が取るのは出来事で、全長 が取るのは物の寸法。の の前が サ変名詞(inflection_classes の
    // suru クラス、15752 語)なら出来事なので 前兆 を優先してよい。道路の全長/吊り橋の全長 は
    // サ変名詞でないので無傷。判定材料は の→全長 5320 と の→前兆 5771 の bigram 差 451 だけで、
    // 前後の語を見ていなかった。加点はその差を超える幅にする。
    // 同じ形(前の語が出来事かで表記が決まる)の語が見つかったら表に足す
    static let multiClauseEventNounAfterSuruNounBonuses: [String: Int] = [
        "前兆": 800
    ]
    // 上の規則を「サ変名詞でないが出来事・状態を表す名詞」へ広げる分(ユーザ指摘 2944:
    // 「炎症になる→炎症の前兆」のように になる 型も 前兆 を取る)。辞書に になる 型の印は無いので
    // 人手の表にする。suru クラスに載っている語(噴火/貧血/発熱/下痢/便秘/老化/停滞/破綻/倒産/中毒/
    // 渋滞/停電/病気 等)は上の判定で拾えるのでここには入れない。足りなければ追加する
    static let multiClauseEventNounsForPrecursor: Set<String> = [
        "炎症", "津波", "地震", "台風", "洪水", "恐慌", "大雨", "豪雨", "猛暑", "寒波",
        "火事", "不況", "赤字", "高熱", "過労", "異変", "異常", "危機", "過敏"
    ]
    ...
    static let multiClauseFinalParticleReadings: Set<String> = ["かな", "かなあ", "かも", "よね", "かしら", "よな", "かー", "ねー", "なー", "よー", "わー", "のー", "かなー", "よねー", "よねえ", "のよね", "のよねー", "し", "な", "ね", "よ", "けど", "けどー", "なあ", "ねえ", "よお", "わあ", "だけ",
        ...
        "のね", "だろー", "だろ", "ろー",
        // でしょー(だめでしょー): 末尾の長音でカタカナ語とみなされ でショー が支配していた(ユーザ報告 2851)
        "でしょー",
        // のか/のかな(疑問の説明終止: こうなるのか)。EOS 未観測で素通りとなり、
        // 人名収穫の 乃佳/乃歌/乃花 が文末で勝っていた(ユーザ報告 2666)
        "のか", "のかな",
        // 想起の っけ(でしたっけ/だったっけ)。無いと でした+っけ の全かな最良がエコー抑制され
        // 変換候補が「候補なし」になっていた(だったっけ は全ノード辞書語で免除されていた。2758)
        "っけ"]
    ...
    // 上の そう に続くかなコピュラ/否定/推量(そうでもない/そうだ/そうじゃない/そうかも)。
    // 文中でも かな そう を全漢字表層(相/層/総)より優先する根拠(適用箇所のコメント参照)
    static let multiClauseSouCopulaFollowerReadings: Set<String> = [
        "で", "でも", "でもない", "でもなかった", "でもなく", "だ", "です", "でした", "じゃ", "じゃない",
        "じゃなかった", "では", "ではない", "ではなかった", "でしょ", "でしょう", "だろ", "だろう",
        "かも", "かな", "なの", "なん", "なんだ", "だね", "だよ", "ですね", "ですよ"
    ]
    ...
    // だったん(2835): 連文節の だったんそば は LM(脱炭 7166<韃靼 7489)で 脱炭そば が先。seed 順(韃靼 先頭)をノードコストへ
    ...
    static let multiClauseKanaAdverbReadings: Set<String> = [
        ...
        "いまだ", "まだ",
        // かなり(副詞、かな正書): 辞書 rank0 が 可(可なり の縮約)で、できたらかなり→できたら可 になっていた(ユーザ指定 2736)
        "かなり",
        ...
        "なぜ",
        // 並列の接続詞(かな正書。公用文の表記ルールでも かな が原則):
        // または/又は は両方 LM 未収録で辞書順(又は 上位)に従い、そーすまたは→ソース又は に
        // なっていた。および は LM 収録(および3978 < 及び4413)で元からかな先頭だったので
        // 同じ扱いに揃える。もしくは/ならびに も同様(ユーザ指定 2026-08-27)
        "または", "もしくは", "および", "ならびに", "あるいは",
        // さすが(副詞、かな正書。seed もかな先頭): 連文節では 指す(LM 4724)+が が
        // さすが(6586)を跨ぎ、さすがあっぷる→指すがApple になっていた(ユーザ報告 2666)
        "さすが",
        // たとえば(接続詞・副詞、かな正書。seed もかな先頭。ユーザ指定 2688)
        "たとえば",
        // たいして(副詞、かな正書。seed もかな先頭): 対し+て(に対して の統計 4511+219)が
        // かな たいして(7772)を跨ぎ たいしてうまくない→対してうまくない だった(ユーザ指定 2771)
        "たいして",
    ...
    static let multiClauseBigramPairBonuses: [String: Int] = [
        ...
        // 差(参加 4500 < 傘下 5560)を食い潰して さんかのしかた→傘下の仕方 が 557 差で勝っていた
        // (ユーザ指定 3203)
        "参加\tの": 800,
        // 言わん+こっちゃ: 慣用句 言わんこっちゃない は misc で 1 ノードだが、打っている途中(いわんこっちゃ/いわんこっちゃな)は
        // イワン(Wikipedia の人名 unigram 6361)+こっちゃ が 言わん(派生 7200)+こっちゃ に 339 差で勝っていた(ユーザ報告 3215)
        "言わん\tこっちゃ": 1000,
        // に+使える: LM は に→仕える 4426 < に→使える 4467(Wikipedia の「神に仕える」偏り)で、文末でない位置
        // (ふつうにつかえるわけ)では 仕える が 41 差で勝っていた。日常語は 使える(ユーザ報告 3109)
        "に\t使える": 400,
        // ぶどう樹の仕立て(垣根仕立て/株仕立て/棒仕立て/一本仕立て。2884、抜き取り検査)。
        // 仕立て 6450 は LM 未観測の連接で 垣根+し(2737)+縦(3630) の断片連鎖に負けていた
        // 暑い+気候・季節の語(かんそうしてあついきこう→乾燥して厚い気候。ユーザ指定 2885)。あつい の LM は
        // 厚い 6243 < 熱い 6467 < 暑い 6763 で無文脈では 厚い。あつい 全体は動かさず後続の語で判断する
        "暑い\t気候": 1500,
        "暑い\t夏": 1500,
        "暑い\t日": 1500,
        "暑い\t季節": 1500,
        ...
        "暑い\t国": 1500,
        "垣根\t仕立て": 1200,
        // 株 は Sudachi サ変の 歌舞する の派生 歌舞した(定額 7200)+て(2591)=9791 に 株(6489)+仕立て(6950) が
        // 3650 差で負けるので、この隣接だけ大きく寄せる
        "株\t仕立て": 3800,
        "棒\t仕立て": 1200,
        "一本\t仕立て": 1200,
        // 経済・政治の語+政策(2873、ユーザ報告 えんやすせいさく→円安制作)。助詞を挟まない隣接
        "円安\t政策": 2000,
        "円高\t政策": 2000,
        "金融\t政策": 2000,
        "経済\t政策": 2000,
        ...
        "農業\t政策": 2000,
        "エネルギー\t政策": 2000,
        // 最+方位(最東部/最東端 等。2880、ユーザ報告 クロアチア最東部→際東部)。最→北部/南部/西部 は
        // 観測済み(2083〜2555)だが 東部 と 〜端 が無く、最 は wc 8884 の常設供給なので 際(4166)との
        // 差 4700 を跨ぐ幅が要る
        "最\t東部": 5500, "最\t東端": 5500, "最\t西端": 5500, "最\t南端": 5500, "最\t北端": 5500,
        // サ変名詞+策(対処策/解決策 等。2880、ユーザ報告 対処策→対処柵)。打開→策 748 は観測済みだが
        // 対処 等が無く 柵(5936)と 策(5709)の僅差を bigram 未観測で落とす
        "対処\t策": 2000, "解決\t策": 2000, "改善\t策": 2000, "対応\t策": 2000, "防止\t策": 2000, "善後\t策": 2000,
        // かき+を は 柿/牡蠣(食べ物)。かき 単独と かきの は 下記 のまま(seed 順)、を が続くときだけ
        // 食べ物側へ。火器→を 821 が観測済みで 柿(6641)/牡蠣(7099)が 火器(6625)に負けていた(2881、ユーザ指定)
        "柿\tを": 2600, "牡蠣\tを": 2300,
        // 山地+気候(ワイン用語の気候区分。さんち の seed 順は 産地 が先なので、
        // 気候 が続くときだけ 山地 に戻す。bigram は未観測。2878)
        "山地\t気候": 2000,
        // 保険+効く(ほけんきくのかな。助詞を挟まない隣接。2850)。隣接表は完全一致なので活用形は別に並べる
        "保険\t効く": 2000,
        "保険\t効いた": 2000,
        "薬\t効く": 2000,
        "薬\t効いた": 2000,
        // 拭く+手間(ふくてまも): →手間 の bigram は 副/服/吹く/拭く とも未観測で遷移が同額(6955)、ふく の語コストだけで
        // 副 6026 < 吹く 6606 < 服 6681 < 拭く 7906 と並び 副手間も が先頭だった(ユーザ報告 2837)。差 1880 を跨ぐ幅
        "拭く\t手間": 2500,
        // の+層(あぶらのそう): 層 は Sudachi に そう 読みの行が無く seed 供給のみ。LM は の→僧 4132 / の→総 4137 /
        // の→相 4579 / の→層 4631 で、寺の僧・総〜・相〜 の Wikipedia 偏りが「Xの層」に勝つ(ユーザ報告 2840)。
        // 僧→EOS 2118 と 層→EOS 2489 の差も含めて跨ぐ幅
        "の\t層": 1500,
        ...
        "の\t席": 1200,
        // せいほうの(ユーザ指定 2729): 西方→の 816 が 製法→の 1128 より強く、unigram(西方 5793<製法 6278)と
        // 合わせて 語+助詞1字 のボーナス 700 でも約100残って 西方の が先頭だった。西方 は2番目で十分
        "製法\tの": 600,
        // かかくをやすくした(ユーザ報告 2741): やすく→し 1817(分かりやすくし 等の 〜やすく 用法の観測)がかな側を押し上げ、
        // を→安く 5209 の優位を下流で覆して 価格をやすくした になっていた。安くし(て/た)の定型を通す
        "安く\tし": 1200,
        // たいりょうり(ユーザ報告 2738): タイ→料理 2538 の観測に対し 鯛→料理 は未観測で、変種の許容差を超えて
        // 鯛料理 が出ず 対料理/たい料理 が並んでいた。タイ料理 は最良のまま、鯛料理 を seed 順変種(2位)に届かせる
        "鯛\t料理": 2500,
        // からくちから(ユーザ報告 2735): から(2848)+口(から→口 4801)+から(口→から 1381)=9530 が
        // 辛口(7040)+から(未観測 500+2848)=10888 に約1350差で勝っていた。辛口から甘口まで の定型を通す
        "辛口\tから": 1600,
        // がいとうするごく(ユーザ報告 2723): する→極 4831+極 5428 が する→語句 5337+語句 6376 を 1454 上回り
        // 該当する極 が先頭だった(を が続けば 語句→を 703 で覆るが、文末では覆らない)。
        // 動詞 する の直後に 極(名詞)が続く文は稀で、〜する語句 は定型
        "する\t語句": 1700,
        // くうしかない(ユーザ報告 2722): 空 5055 < 食う 6911 の unigram 差で 空しかない が先頭だった
        // (食う→しか/空→しか とも未観測)。〜しかない は動詞終止形に付く慣用が主で、空しかない は
        // 手動選択+学習で救済
        "食う\tしか": 2200,
        ...
        "お\t待ち": -3000,
        // もうまくも(2819): かな副詞 もう(5463)+膜(遷移 1980)が 網膜(7579)+も と 136 差で並び、もう膜も が勝つことがある。
        ...
        "行\t返る": 1500,
        // しゃしんとって(2828): を落ちの 写真+動詞 で 取って/撮って が同点になり列挙順で 取って が先頭になる。
        // 写真 の動詞は 撮る(Sudachi は 撮っ+て に分割し bigram が無い)
        "写真\t撮って": 800,
        "写真\t撮った": 800,
        "写真\t撮る": 800
    ]
    ...
    // を を落とした口語の 名詞+動詞(ログ取って/メモ取って/写真撮って)。名詞→を の bigram が強い名詞の直後の漢字活用派生を
    // 割り引く。ろぐとって で 名詞+名詞 ログ取手(7166、EOS 2119)が ログ取って(派生 7200、て形の EOS 2675)に 90 差で
    // 勝っていた(ユーザ報告 2828)。かな派生(とって)には掛けない(漢字の動詞を先頭にしたい)
    static let multiClauseWoDropVerbMaxWoBigram = 1500
    static let multiClauseWoDropVerbBonus = 1500
    // を落ちの対象外にする自動詞・存在動詞の活用読み(居る/有る/成る)。を の目的語は他動詞にしか付かないので、
    // ヒト+居ない(いれているひといないかなあ→入れているヒト居ないかなあ)のような 名詞+自動詞 は割り引かない
    ...
    static let multiClauseColloquialExplanatoryTailReadings: Set<String> = [
        ...
        // 単文節に かなー 受け皿あり。かなあ は misc の curated が受け皿(3081)
        "かなー", "かなあ"
    ...
    static let multiClauseInflectionAuxTails: [String] = [
        // 受身・可能の られる/れる(A単位は 答え+られる)。答えられる(派生 7200)+よう が られる→よう(1326)を
        // 借りられず、応え+られる(応え→られる の観測 bigram)+よう の 3 分割に負けて こたえられるよう が
        // 応えられるよう になっていた(ユーザ報告 3246)。派生ノード限定(auxTailForBigramBorrow)
        "られる", "れる",
    ...
    static let multiClauseBigramBorrowDeniedReadingsBySurface: [String: Set<String>] = [
        ...
        "輪": ["わ"],
        // 歯(ば)は連濁読み(入れ歯/前歯/奥歯)の収穫で、後部要素専用。単独では文中に立たない。
        // 医(い)も熟語専用(医学/名医)。医→歯 の bigram(1394=医歯薬 のA単位分割)を借用して
        // 「あおいばぶる→青+医+歯+ブル」のような単漢字断片連鎖を作り、青い+バブル に勝っていた
        // (wc は ば 7327 > 主読み は 5745。読み1字なので短span床(読み3字以上)では捕まらない。2859)
        "歯": ["ば"],
        "医": ["い"],
    ...
    // もん は 物 の話し言葉(うまいもん/やすいもん)。LM は 門 5199 < もん 5938 で、い形容詞の後に bigram が無いと
    // 門 が勝っていた(うまいもん→うまい門。ユーザ報告 3108)
    // わけ(使えるわけがない)も述語直後はかなが正書(訳 は文語・翻訳の意で残す。3109)
    static let multiClauseFormalNounKanaReadings: Set<String> = ["とき", "こと", "もの", "もん", "ため", "だけ", "はず", "やつ", "くせ", "わけ"]
    static let multiClauseFormalNounKanjiPenalty = 1000
    static let multiClauseFormalNounKanaBonus = 800
    ...
    // 上の「直後」は 1 字の助詞ノードとは限らない。curated の になる が 1 ノードで立つと 経ん+になる になり
    // 減点を素通りしていた(たかさがへんになる。実機のみ=テストは misc を読まない。3122)。読みの先頭で見る
    static let multiClauseNegativeNFollowingParticleHeads: Set<Character> = ["に", "を", "へ", "が", "は"]
    ...
    static let multiClauseKanaIdentityFloorExemptReadings: Set<String> = [
        ...
        "うまい",
        // なる(補助動詞・変化動詞、かな正書): LM は なる 3405 ≪ 成る 5822 なのに、かな識別の
        // 短span床(wc 8021)で 成る(7966)より重くなり なるといいなー→成るといいなー、
    ...
    // 述語+と の直後の いった/いって/いう は引用の 言った(行くと言った後で。2881、ユーザ報告)。
    // 行った/言った は派生同士の同点で列挙順(行 が先)になっていた。名詞+と(マラガといった)は列挙で
    // かなにクランプ済み(2878)なので、述語直後に限る
    ...
    // 文頭の判定はノードで見る(2885): 文字で見ると後続のカタカナ語・複合語の頭文字(ハイブリッド/東部/長野県)を
    // 助詞と誤認し、ははいぶりっどひんしゅで→歯ハイブリッド品種で 等が素通りしていた(抜き取り検査 14 件)。
    // 直後のノードがこのかな助詞(表層==読み)のときだけ名詞用法(歯を磨く/歯が痛い)と見なす
    static let multiClauseParticleFollowerSurfaces: Set<String> = [
        "は", "が", "を", "に", "と", "も", "で", "へ", "の", "や", "か", "ね", "よ", "な",
        "には", "では", "とは", "から", "まで", "より", "でも", "って"
    ]
    // 述語(辞書形/た形)直後の形式名詞 とき(去るときに/帰ったとき)。さる の LM は 猿 5848 < 去る 6307 で、
    // 猿ときに(名詞+とき は の が要る非文)が 去るときに に勝っていた(ユーザ報告 2887)。連体修飾を受ける
    // とき/時 の読み頭で判定し、表層(とき/時)は問わない
    static let multiClauseTokiAfterPredicateBonus = 1500
    ...
    static let multiClauseBigramPrefixPairBonuses: [String: Int] = [
        ...
        // 単文節では seed で先頭(ユーザ指定)のまま、を/が の後の動作(更新した/更新する)だけ 更新 に寄せる
        "を\t更新": 2500,
        // を+来る族(服を来た/を来て)は非文(来る は自動詞)。くる 族を派生の先頭にした(2910)ので負のボーナスで目的語の後の
        // 着る 族を守る(を→来た は curated 来た が床 1500 で立ち、派生 着た 5000 より安い。3000 では足りない)
        "を\t来": -4500,
        "が\t更新": 2500,
        // に+指示(えいあいにしじ→AIに師事。ユーザ報告 2910)。bigram は に→支持 4370 / 師事 4410 / 指示 4604 で Wikipedia の
        // 人物記事(〜に師事)寄り。フリック入力では 〜に指示(する) が日常
        "に\t指示": 800,
        // 来た+道(かれのきたみち→彼の北道。ユーザ指定のスポット対処 2892)。来た(misc curated)は短い断片の床外し
        // (北見 が長語にある)で派生定額 7200 に落ち、の→北 4326+道 に負ける。北 を下げると 北未知/北道 に逃げるので
        // 来た+道 の隣接を直接持ち上げる
        "来た\t道": 3500,
        // 格助詞+痛(はがいたい→歯が居たい。2893)。痛い の unigram は 7302 と高く、居たい/射たい/鋳たい(派生、助詞直後
        // 5000)に負ける。が/は/も の直後の いたい は 痛い(居たい は に の後: ここにいたい)
        "が\t痛": 3500,
        "は\t痛": 3500,
        "も\t痛": 3500,
        // 自動+サ変名詞(じどうかくてい→児童確定。同上)。児童 5016 と 自動 5047 は unigram 差 31 で bigram 未観測
        "自動\t確定": 1000, "自動\t変換": 1000, "自動\t更新": 1000, "自動\t保存": 1000, "自動\t入力": 1000,
        "自動\t再生": 1000, "自動\t調整": 1000, "自動\t補完": 1000, "自動\t削除": 1000, "自動\t設定": 1000,
        "自動\t送信": 1000, "自動\t検出": 1000, "自動\t運転": 1000, "自動\t切り替": 1000, "自動\t修正": 1000
    ]
    // Wikipedia 偏りの同音語の降格(読み → [表層: 加算])。連文節でだけ、LM が百科事典寄りで前に出す表層を
    // 日常入力の頻度実感に合わせて後ろへ下げる(マニュアル検査でユーザ指定 2890)。単文節の並びは seed 側で揃える。
    // 官僚 4987 vs 完了 5182 / 呼称 4946 vs 故障 5654(胡椒 は触らない)/ 大気 5289 vs 待機 6086 /
    ...
    static let multiClauseHomophoneDemotionsByReading: [String: [String: Int]] = [
        "かんりょう": ["官僚": 800],
        // 扈従/誇称 は派生定額で 故障する と同点になり文字コード順で 扈従する が先頭に出る(2892)
        "こしょう": ["呼称": 1200, "扈従": 1200, "誇称": 1200],
        "たいき": ["大気": 1200],
        "たいひ": ["対比": 1000],
        "じんめい": ["人命": 600]
    ]
    ...
    static let multiClausePrenominalVerbNounPreferences: [String: String] = ["かんせい": "感性"]
    static let multiClausePrenominalVerbNounBonus = 2500
    ...
    // 助詞の後は正当なので、直前がかな以外の体言のときだけ減点する
    static let multiClauseParticleIiSwallowingReadings: Set<String> = ["はいい", "がいい", "をいい", "にいい", "といい", "もいい"]
    static let multiClauseParticleIiSwallowingPenalty = 2500
    // 来た(misc curated。床 1500/文頭 700)は は/を の直前と の+文末では立たない(来たは/来たを は非文、
    // その来た/ベーズの来た は連体で後続が要る)。北 に譲る(2885、抜き取り検査 32 件)
    static let multiClauseKitaCuratedNonPredicatePenalty = 5000
    ...
    // な(連体・終助詞)も文頭には立たない: なにするひと が な+にする(curated 床 1500)+人 で 何する人 を消していた(2840)
    static let multiClauseBOSPenalizedParticles: Set<String> = ["は", "が", "を", "へ", "も", "に", "か", "かね", "て", "な"]
    ...
    // 姓/性 は候補に残る
    // ぶどう も同型(2878、ユーザー指定)。LM は ブドウ 6075 < 葡萄 6625 < ぶどう 6808 で
    // 連文節が ブドウ産地です を先頭にしていた。かなを先頭に、以降は LM 順(ブドウ→葡萄)
    // もっとも もユーザー指定でかな正書(2879)。LM は 最も 4371 ≪ もっとも 5139 で
    // 連文節が 最も を先頭にしていた。単文節の並びは動かさないので 最も/尤も は候補に残る
    ...
    static func auxTailForBigramBorrow(of node: MultiClauseNode) -> String? {
        ...
        // られる/れる も派生ノード限定。かな識別(ふれる/これる 等)に借用させると名詞・かな語の評価が歪む
        if tail == "られる" || tail == "れる", !node.isInflectionDerived {
            return nil
        }
        return tail
    }
