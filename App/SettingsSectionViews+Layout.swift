...
struct DirectionSettingsSection: View {
    ...
    var body: some View {
        SegmentedSettingsCard(
            ...
            style-hanabi は 右=い/下=う/左=え/上=お(3 時方向から時計回り。1998 年の Newton OS 向け入力方式 Hanabi と同じ)。

            style-i は 左=い/上=う/右=え/下=お(9 時方向から時計回り。iPhone 標準キーボードと同じ)。

            style-écritu は 上=い/右=う/左=え/下=お(時計回りではなく、12 時方向から Z 字順)。
