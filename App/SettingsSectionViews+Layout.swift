...
struct DirectionSettingsSection: View {
    ...
    var body: some View {
        SegmentedSettingsCard(
            ...
            hanabi式は 右=い/下=う/左=え/上=お(3 時方向から時計回り。1998 年の Newton OS 向け入力方式 Hanabi と同じ)。

            littlebear式は 左=い/上=う/右=え/下=お(9 時方向から時計回り。2008 年以来 iPhone の標準キーボードで使われている並び)。

            écritu式は 上=い/右=う/左=え/下=お(時計回りではなく、12 時方向から Z 字順)。
