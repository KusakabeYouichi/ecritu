...
struct KaCounterVariantSettingsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("数を数える言い方の『か』(1か所、数か月、何か国 など)を、どの表記で、どの順に出すかを決めます。行を長押しして動かすと順序が変わり、スイッチを切った表記は候補に出しません。初期設定は か だけを出します(保守的初期設定では か・箇・ヶ・カ)。並びの初期値は か → 箇 → ヶ → カ → ヵ → 個 → ケ です。\n\n公用文では『か所・か月』または『箇所・箇月』と書くのが標準で、新聞は『か所・か月』です。ここで決めた順は変換候補の並びの初期状態で、いつも選ぶ表記は学習によってその上に出ます。")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
...
struct OkuriganaVariantSettingsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("送り仮名には本則のほかに許容された書き方があります(終わる⇄終る、取り扱う⇄取扱う、届け⇄届)。許容形を候補に出すか、出すならどちらを先にするかをグループごとに決めます。初期設定はどのグループも本則だけを出します(保守的初期設定では本則を先に許容形も出します)。\n\n慣用で送り仮名を付けない語(受付、取引、割引、申込、日付、番組、入口、締切 など)と、名詞として送らない語(話、光、係、組、次、隣 など)はここでの対象外で、常にそのまま出ます。学習した表記と追加語彙に登録した表記は、この設定に関わらず候補に出ます。出荷時の追加語彙には 行なう がこの例外として登録してあります(不要なら追加語彙から削除してください)。")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
