...
final class KanaKanjiConverterRegressionTests: XCTestCase {
    ...
    func testRegressionRealLMSweepCustomOrderReadings() throws {
        ...
        let expectations: [(String, [String])] = [
            ...
            ("かこう", ["加工", "書こう", "描こう"]),
            ("かろう", ["刈ろう", "家老"]),
            ("こうこ", ["公庫", "香茹", "考古"]),  // 2927 の目視で書き言葉側(公庫)を先頭に
            ("こうし", ["仔牛", "講師", "行使", "格子", "孔子", "光子"]),
            ("こうしん", ["更新", "香信", "行進"]),  // 2927 の目視で書き言葉側(更新)を先頭に
            ("こんだ", ["混んだ", "込んだ", "今田"]),
            ("しんとう", ["浸透", "神道", "心頭", "震盪"]),
            ("するっ", ["するっ", "スルっ", "スルッ"]),
            ("せいい", ["誠意", "征夷", "青衣"]),
    ...
    func testRegressionRealLMSweepApprovedResiduePairs() throws {
        ...
        let pairs: [(String, String)] = [
            ...
            ("かき", "牡蠣"), ("かぐら", "神楽"),  // かき は 2963 の話し言葉側の目視で 牡蠣 に ("かごう", "化合"), ("かずお", "和夫"), ("かずき", "和樹"),
    ...
    func testRegressionRealLMWordCostAnomalyBatchOrdering() throws {
        ...
        let expectations: [(String, [String])] = [
            ...
            ("かん", ["缶", "勘", "観"]),
            ("ふじた", ["藤田"]),
            ("かき", ["牡蠣", "柿", "下記", "書き"]),  // 2963 の話し言葉側の目視で 牡蠣 を先頭に
            ("みる", ["見る", "観る", "みる", "診る"]),
            ...
            ("ちじょうは", ["地上波"]),
            ("やしき", ["屋敷", "邸"]),
            ("こうち", ["耕地", "高知", "高地"]),  // 2927 の目視で書き言葉側(耕地、misc 登録)を先頭に
            ("ながのし", ["長野市"]),
            ("にっせい", ["ニッセイ", "日生", "日成"]),
            ("いわた", ["岩田", "イワタ"]),
            ("および", ["および", "及び", "及"]),
    ...
    func testRegressionRealLMKakokuCounter() throws {
        ...
        // 表示層と同じく、数字直後ブーストの後に か の表記設定を通す(ここでは保守的初期設定 か→箇→ヶ→カ)
    ...
    func testRegressionRealLMKagetsuKagakiFirst() throws {
        ...
        XCTAssertEqual(kakoku, ["数か国"], "kakoku=\(kakoku)")  // 戦略的初期設定は か のみ(2817)
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionRealLMBatch2801Readings() throws {
        ...
        // 2802: 区切り違いの 目標都市 を第2候補として残す(と+し 2ノード→都市 1ノードの代替経路)
        XCTAssertEqual(Array(toshi.prefix(2)), ["目標とし", "目標都市"], "toshi=\(toshi)")
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionRealLMKaCounterCompounds() throws {
        ...
        // 六ケ所(地名)は 2816 の か 表記設定で 六か所 の表記群に畳まれる(ヶ をオンにすると 六ヶ所 として出る)
        XCTAssertEqual(converter.candidates(for: "ろっかしょ", limit: 10, systemCandidateMode: .surface), ["6か所", "六か所"])
        converter.setKaCounterVariantPreference(.conservative)
        defer { converter.setKaCounterVariantPreference(.default) }
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionRealLMKaCounterVariantPreference() throws {
        ...
        // 保守的初期設定(か・箇・ヶ・カ)
        converter.setKaCounterVariantPreference(.conservative)
        let conservativeOrder = ["か", "箇", "ヶ", "カ"]
        XCTAssertEqual(converter.candidates(for: "いっかしょ", limit: 8, systemCandidateMode: .surface), conservativeOrder.map { "1\($0)所" })
        XCTAssertEqual(converter.multiClauseCandidates(for: "すうかこくたいおう", systemCandidateMode: .surface), conservativeOrder.map { "数\($0)国対応" })
...
extension KanaKanjiConverterRegressionTests {
    // 2885: コスト・費用の語+抑える(ユーザ指定。コストを押さえた)。手を押さえた は無傷
    func testRegressionOsaeruAfterCostNoun() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)
...
extension KanaKanjiConverterRegressionTests {
    // 2889: 指示副詞+と+いった は引用(かれはそうといった→彼は相といった)
    func testRegressionSouToItta() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)
...
extension KanaKanjiConverterRegressionTests {
    // 2890: 段(だん)を短span床から免除(マニュアル検査 だんでならびます→ダンで並びます 14 件)
    func testRegressionDanFloorExempt() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)
...
extension KanaKanjiConverterRegressionTests {
    // 2890: 自動+サ変名詞の隣接ボーナス(じどうかくてい→児童確定)
    func testRegressionJidouCompounds() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionKoushinAfterParticle() throws {
        ...
        for mode in [KanaKanjiCandidateSourceMode.normalise, .surface] {
            ...
            // 2551 は 香信(乾しいたけの等級)を先頭にしていたが、2927 の目視判定で書き言葉側の 更新 に
            XCTAssertEqual(converter.candidates(for: "こうしん", limit: 3, systemCandidateMode: mode).first, "更新", "mode=\(mode.rawValue)")
        }
    }
}
...
extension KanaKanjiConverterRegressionTests {
    ...
    // 話し言葉優先は既定で、個別の目視判定はそれに優先する。5 倍の帯の 4 読み
    // (いじ/えんしゅう/せんこう/とうにゅう)は逆に話し言葉側へ戻した
    func testRegressionSpokenRegisterHomophoneOrder() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)

        let expectations: [(String, [String])] = [("いし", ["意思", "医師", "石", "意志", "遺志"]), ("だいたい", ["大体", "代替"]), ("きのう", ["きのう", "昨日", "機能"]), ("しゅうかん", ["週間", "習慣"]), ("かし", ["菓子"]), ("じてん", ["自転", "時点"]), ("しょうちゅう", ["焼酎", "尚忠"]), ("かんせん", ["幹線", "感染"]), ("じこ", ["自己", "事故"]), ("きじ", ["生地", "記事"]), ("こうほう", ["広報", "工法"]), ("ちゅうしゃ", ["駐車", "注射"]), ("かいじょう", ["会場", "海上"]), ("がっき", ["楽器", "学期"]), ("せんたく", ["選択", "洗濯"]), ("かがく", ["科学", "化学"]), ("こうか", ["効果", "高価"]), ("せいかく", ["性格", "正確"]), ("いらい", ["依頼", "以来"]), ("せいしん", ["精神", "清新"]), ("たいちょう", ["体調", "隊長"]), ("ぼうし", ["帽子", "防止"]), ("たいしょう", ["対象", "大正"]), ("へんしん", ["返信", "変身"]), ("ようじ", ["用事", "幼児"]), ("きょうぎ", ["協議", "競技"]), ("せいふく", ["制服", "征服"]), ("きげん", ["期限", "起源"]), ("はんちょう", ["班長", "藩庁", "半丁"]), ("めいし", ["名刺", "名詞"]), ("きょうせい", ["強制", "共生"]), ("ぎじ", ["議事", "疑似"]), ("こうせき", ["功績", "鉱石"]), ("こうりつ", ["効率", "公立"]), ("たいきょく", ["大局", "太極", "対局"]), ("かんじょう", ["感情", "勘定"]), ("しょうてん", ["商店", "焦点"]), ("しんり", ["心理", "真理"]), ("せいきょう", ["盛況", "生協", "正教"]), ("しんちょう", ["慎重", "身長", "伸長"]), ("きせい", ["規制", "寄生"]), ("いじ", ["意地", "維持"]), ("きょうよう", ["教養", "強要"]), ("りこう", ["履行", "利口", "理工"]), ("あさいち", ["朝市", "朝一"]), ("かっぱ", ["河童"]), ("きき", ["危機", "機器"]), ("こうひょう", ["公表", "好評", "講評"]), ("せんこう", ["線香", "選考"]), ("いちょう", ["銀杏", "胃腸", "いちょう", "萎凋"]), ("こうてい", ["肯定", "皇帝", "工程"]), ("しぼう", ["脂肪", "死亡"]), ("ぼうちょう", ["膨張", "傍聴"]), ("かいひ", ["会費", "回避"]), ("こどう", ["鼓動", "古道", "跨道"]), ("じきゅう", ["時給", "自給"]), ("とうにゅう", ["豆乳", "投入"]), ("しょうがい", ["障害", "生涯", "障碍", "渉外"]), ("じょうすい", ["上水", "浄水"]), ("とうこう", ["登校", "投稿"]), ("はんてん", ["反転", "半纏"]), ("ようりょう", ["容量", "要領"]), ("さんどう", ["参道", "賛同"]), ("ていじ", ["定時", "提示"]), ("こしつ", ["個室", "固執"]), ("えんしゅう", ["円周", "演習"]), ("かんぱち", ["環八", "間八"]), ("こうしゃ", ["後者", "校舎"]), ("ふじん", ["婦人", "夫人", "布陣", "不尽"]), ("ようせい", ["養成", "要請"])]
        for mode in [KanaKanjiCandidateSourceMode.normalise, .surface] {
    ...
    func testRegressionUserReports2973() throws {
        ...
        for mode in [KanaKanjiCandidateSourceMode.normalise, .surface] {
            ...
            XCTAssertEqual(
                ...
                "棚に置きます", label)
            XCTAssertEqual(
                converter.multiClauseCandidates(for: "ほんをおきますか", systemCandidateMode: mode).first,
                "本を置きますか", label)
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionSpokenCorpusHomophoneOrder() throws {
        ...
        let expectations: [(String, String)] = [
            ("かき", "牡蠣"), ("かいき", "回忌"), ("しこう", "思考"), ("いちょう", "銀杏"),
            ("ほうか", "放課"), ("ようし", "用紙"), ("とうき", "陶器"), ("かみがた", "髪型")
        ]
        for mode in [KanaKanjiCandidateSourceMode.normalise, .surface] {
            for (reading, expected) in expectations {
                XCTAssertEqual(
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionWrittenCorpusHomophoneOrder() throws {
        ...
        let expectations: [(String, String)] = [
            ("こうか", "効果"), ("たいさく", "対策"), ("だいじん", "大臣"), ("すいしん", "推進"),
            ("せいしん", "精神"), ("ごぜん", "午前"), ("しほん", "資本"), ("かだい", "課題"),
            ("なかま", "仲間"), ("しんせい", "申請"),
            // 辞書に表層が無く misc.plist に登録した分
            // いちょう は 2963 の話し言葉側の目視で 銀杏 に戻した(下の話し言葉側のテストで固定)
            ("ろうし", "労使"), ("こうち", "耕地"), ("かちゅう", "家中"),
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testRegressionSoukaPrefersKanaThenSoka() throws {
        try prepareRealLMDictionary()
        try loadDeviceAddedVocabulary(includeSuppression: true)

        for mode in [KanaKanjiCandidateSourceMode.normalise, .surface] {
            let candidates = converter.candidates(for: "そうか", limit: 8, systemCandidateMode: mode)
            XCTAssertEqual(Array(candidates.prefix(3)), ["そうか", "草加", "創価"], "mode=\(mode.rawValue)")
            XCTAssertFalse(candidates.contains("創\u{25CF}"), "mode=\(mode.rawValue)")
        }
    }
...
extension KanaKanjiConverterRegressionTests {
    ...
    func testKanaOrthodoxAndSeedSuppliedWords() throws {
        ...
        XCTAssertEqual(converter.candidates(for: "したい", limit: 3, systemCandidateMode: .surface).first, "したい")
        XCTAssertEqual(converter.candidates(for: "したいこと", limit: 3, systemCandidateMode: .surface).first, "したいこと")
