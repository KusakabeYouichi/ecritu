...
作成: 2026-09-01、検証 2026-09-19(フリック方向の呼称・キーワード・カテゴリー・著作権を更新)。文字数制限を検証済み。ASCの入力欄にそのまま貼り付け可。
...
『?』『。』『〜』『…』括弧まで、かなのまま打てる日本語フリックキーボード。通信は一切なし — 入力内容が端末の外に出ることはありません。連文節変換、絵文字・顔文字・記号パネル、単位や日付の入力補助まで、1 つのキーボードで。

## キーワード(100字以内・現在71字)

エクリチュ,日本語入力,フリック,かな,IME,連文節,変換,絵文字,顔文字,記号,オフライン,辞書,旧仮名,部首,単位,フランス語

## 説明文(4000字以内)

écritu(エクリチュ)は、『毎回同じことを選ばされる操作をなくす』ことを第一に設計された iPhone 用の日本語フリック入力キーボードです。『?』を打つたびに全角か半角かを選ぶ、句読点や括弧を打つたびに候補から確定する、といった手間を省き、記号は打った瞬間に確定します(どの形で入るかは設定で決めておきます)。そのうえで、よく使う記号のいくつかをかな入力の画面に置き、モードの切り替えそのものも減らしています。
...
■ モードを切り替えずに、そのまま打てる
『?』『!』『。』『・』は読点キーのフリックに、『〜』『ー』はわキーに、括弧はやキーに。よく使う記号のためにいちいち記号モードへ入る必要はありません。やキー・わキーは『2段フリック』に対応し、() や「」などの括弧、旧仮名の『ゐ』『ゑ』まで指を離さずに入力できます。
...
■ 欧文もサジェスト
フランス語・ドイツ語・イタリア語・英語の入力補完を搭載(オフライン辞書)。アクセント付き文字も長押しで。メールアドレスやURLの欄では自動的に英字配列で開きます。
